\documentclass{../vespers-sheet}

\begin{document}

%% TODO: correct date
\chapter*{Second Vespers of St. Nicholas, Bishop and Confessor}

\begin{center}
\includegraphics[width=0.7\linewidth]{st-nicholas}
\end{center}

\vfill\newpage

\begin{rubricbox}

{\color{red}When the Officiant kneels, all \textbf{kneel} and pray silently.
Then, when the Officiant stands, all \textbf{stand} and say silently one \textit{Pater noster} (Our Father) and \textit{Ave Maria} (Hail Mary).
Then all make the sign of the cross with the Officiant as he intones:}

\end{rubricbox}

\gresetinitiallines{1}
\gregorioscore{../common/deus-in-adjutorium-solemn}

\textit{
O God, come to my assistance.
{\color{red}\Vbar.}~O Lord, make haste to help me.
Glory be to the Father, and to the Son, and to the Holy Spirit,
as it was in the beginning, is now, and ever shall be, world without end. Amen.
Praise to Thee, O Lord, King of endless glory.}

%%

\section*{Psalm 109}

\textit{\textnormal{Ant. 1.} Behold an high priest, who in his days pleased God, and was found righteous.
 \textnormal{Ps.} The Lord said to my Lord...}
 
\begin{rubricbox}

{\color{red} All stand during antiphon; the Officiant intones it and all \textbf{sit} at the asterisk. The Cantor intones the Psalm verse.}

\end{rubricbox}

\gresetinitiallines{1}
\gregorioscore{ps109-antiphon}

\gresetinitiallines{0}
\gregorioscore{ps109-intonation}

\begin{latinenglishsection}

\latinenglish{
	\input{../psalms/ps109-7} %%
}{
	\input{../psalms/english/ps109} %%
}

\end{latinenglishsection}

\gresetinitiallines{1}
\gregorioscore{ps109-antiphon}

%%

\section*{Psalm 110}

\textit{\textnormal{Ant. 2.} None was found like unto him, to keep the Law of the Most High.
 \textnormal{Ps.} I will praise thee, O Lord, with my whole heart...}
 
\begin{rubricbox}

{\color{red} For the remaining Psalms, all proceeds as in the previous Psalm, except that the \textbf{cantor alone} stands when intoning both the antiphon and the Psalm verse.}

\end{rubricbox}

\gresetinitiallines{1}
\gregorioscore{ps110-antiphon}

\gresetinitiallines{0}
\gregorioscore{ps110-intonation}

\begin{latinenglishsection}

\latinenglish{
	\input{../psalms/ps110-7} %%
}{
	\input{../psalms/english/ps110} %%
}

\end{latinenglishsection}

\gresetinitiallines{1}
\gregorioscore{ps110-antiphon}

%%

\section*{Psalm 111}

\textit{\textnormal{Ant. 3.} Therefore the Lord assured him by an oath that He would multiply his seed among His people.
 \textnormal{Ps.} Blessed is the man that feareth the Lord...}

\gresetinitiallines{1}
\gregorioscore{ps111-antiphon}

\gresetinitiallines{0}
\gregorioscore{ps111-intonation}

\begin{latinenglishsection}

\latinenglish{
	\input{../psalms/ps111-8} %%
}{
	\input{../psalms/english/ps111} %%
}

\end{latinenglishsection}

\gresetinitiallines{1}
\gregorioscore{ps111-antiphon}

%%

\section*{Psalm 112}

\textit{\textnormal{Ant. 4.} O all ye Priests of God, bless ye the Lord O all ye servants of the Lord, sing praises unto our God.
 \textnormal{Ps.} Praise the Lord, ye children...}

\gresetinitiallines{1}
\gregorioscore{ps112-antiphon}

\gresetinitiallines{0}
\gregorioscore{ps112-intonation}

\begin{latinenglishsection}

\latinenglish{
	\input{../psalms/ps112-7} %%
}{
	\input{../psalms/english/ps112} %%
}

\end{latinenglishsection}

\gresetinitiallines{1}
\gregorioscore{ps112-antiphon}

%%

\vfill\pagebreak

\section*{Psalm 131}

\textit{\textnormal{Ant. 5.} Good and faithful servant, enter thou into the joy of thy Lord.
 \textnormal{Ps.} Lord, remember David...}

\gresetinitiallines{1}
\gregorioscore{ps131-antiphon}

\gresetinitiallines{0}
\gregorioscore{ps131-intonation}

\begin{latinenglishsection}

\latinenglish{
	\input{../psalms/ps131-3-antiquo} %%
}{
	\input{../psalms/english/ps131} %%
}

\end{latinenglishsection}

\gresetinitiallines{1}
\gregorioscore{ps131-antiphon}

%%

%% TODO: correct the little chapter
 \section*{Little Chapter (Sir 44:16-17)}

\textit{\color{red}All stand. The Officiant leads the Little Chapter:}

\begin{latinenglishsection}

 \latinenglish{
	 Ecce sacérdos magnus, qui in diébus suis plácuit Deo, et invéntus est justus: {\color{red}\GreDagger}\ et in témpore iracúndiæ factus est reconciliátio.\\
 	{\color{red}\Rbar.}~Deo gratias.
 }{
	Behold an high priest, who in his days pleased God, and was found righteous, and in the time of wrath he was made a propitiation.\\
 	{\color{red}\Rbar.}~Thanks be to God.
 }
 
\end{latinenglishsection}

%% TODO: correct the hymn using the antiphonary
\section*{Hymn}

\textit{\color{red}The Officiant intones the Hymn:}

\gresetinitiallines{1}
\gregorioscore{../hymns/iste_confessor}

\vfill
\pagebreak

{\itshape

1. This the confessor of the Lord, whose triumph
Now all the faithful celebrate, with gladness
Erst on this feast-day merited to enter
Into his glory.

2. Saintly and prudent, modest in behaviour,
Peaceful and sober, chaste was he, and lowly.
While that life’s vigour, coursing through his members,
Quickened his being.

3. Sick ones of old time, to his tomb resorting,
Sorely by ailments manifold afflicted,
Oft-times have welcomed health and strength returning,
At his petition.

4. Whence we in chorus gladly do him honour,
Chanting his praises with devout affection,
That in his merits we may have a portion,
Now and forever.

5.His be the glory, power and salvation,
Who over all things reigneth in the highest,
Earth’s mighty fabric ruling and directing,
Onely and Trinal.
Amen.

}

\textit{\color{red}The Cantor says the following before all reply afterwards:}

\gresetinitiallines{0}
\gabcsnippet{
(c3) <c><sp>V/</sp>.</c> Jus(h)tum(h) de(h)dú(h)xit(h) Dó(h)mi(h)nus(h) per(h) vi(h)as(h) rec(h)tas.(g_hvGF'Efgf.) (::)
}

\gresetinitiallines{0}
\gabcsnippet{
(c3) <c><sp>R/</sp>.</c> Et(h) os(h)tén(h)dit(h) il(h)li(h) reg(h)num(h) De(h)i.(g_hvGF'Efgf.) (::)
}

\textit{{\color{red}\Vbar.}~The Lord guided the just in right paths.
{\color{red}\Rbar.}~And showed him the kingdom of God.}

%% TODO: correct the magnificant antiphon and tone - add pointed text to the intonation if needed

\begin{center}
\includegraphics[width=0.65\linewidth]{st-nicholas_art}
\end{center}

\pagebreak

\section*{Magnificat}

\textit{\textnormal{Ant.} The Lord loved him and beautified him. He clothed him with a robe of glory, and crowned him at the gates of Paradise.}

\begin{rubricbox}

{\color{red}The Officiant intones the antiphon; all cross themselves as the Cantor intones the first verse.}

\end{rubricbox}

\gresetinitiallines{1}
\gregorioscore{magnificat-antiphon}

\gresetinitiallines{0}
\gregorioscore{magnificat-intonation}

 \begin{latinenglishsection}

\latinenglish{	
3. \textit{Quia} respéxit humilitátem an\textbf{cíl}læ \textbf{su}æ:~* ecce enim ex hoc beátam me dicent omnes gene\textit{ra}\textit{ti}\textbf{ó}nes.

4. \textit{Quia} fecit mihi \textbf{ma}gna qui \textbf{pot}ens est:~* et sanctum \textit{no}\textit{men} \textbf{e}jus.

5. \textit{Et mi}sericórdia ejus a progénie \textbf{in} pro\textbf{gé}nies~* timén\textit{ti}\textit{bus} \textbf{e}um.

6. \textit{Fecit} poténtiam in \textbf{brá}chio \textbf{su}o:~* dispérsit supérbos mente \textit{cor}\textit{dis} \textbf{su}i.

7. \textit{Depó}suit pot\textbf{én}tes de \textbf{se}de,~* et exal\textit{tá}\textit{vit} \textbf{hú}miles.

8. \textit{Esu}riéntes im\textbf{plé}vit \textbf{bo}nis:~* et dívites dimí\textit{sit} \textit{in}\textbf{á}nes.

9. \textit{Suscé}pit Israël \textbf{pú}erum \textbf{su}um,~* recordátus misericór\textit{di}\textit{æ} \textbf{su}æ.

10. \textit{Sicut} locútus est ad \textbf{pa}tres \textbf{nos}tros,~* Abraham et sémini e\textit{jus} \textit{in} \textbf{s\'{\ae}}cula.

11.  {\color{red}\textit{(bow)}} \textit{Glóri}a \textbf{Pa}tri, et \textbf{Fí}lio,~* et Spirí\textit{tu}\textit{i} \textbf{Sanc}to.

12. {\color{red}\textit{(rise)}} \textit{Sicut} erat in princípio, et \textbf{nunc}, et \textbf{sem}per,~* et in s\'{\ae}cula sæcu\textit{ló}\textit{rum}. \textbf{A}men.  {\color{red}\textit{(sit)}} 
}{	
	\input{../psalms/english/magnificat}
}

\end{latinenglishsection}

%{\color{red}All sing the antiphon together while seated.}

\gresetinitiallines{1}
\gregorioscore{magnificat-antiphon}

%% TODO: proper collect for the date

\section*{Collect}
\textit{\color{red}\textbf{All stand.} The Officiant leads the Collect:}

\begin{latinenglishsection}

\latinenglish{
	{\color{red}\Vbar.}~Dómine exáudi oratiónem meam.\\
	{\color{red}\Rbar.}~Et clamor meus ad te véniat.
	
	Orémus.
	Deus, qui beátum Nicoláum Pontíficem innúmeris decorásti miráculis: tríbue, qu\'{\ae}sumus; ut ejus méritis et précibus a gehénnæ incéndiis liberémur.\\
	% Per Dóminum nostrum Jesum Christum, Fílium tuum: qui tecum vivit et regnat in unitáte Spíritus Sancti, Deus, per ómnia s\'{\ae}cula sæculórum.
	% {\color{red}\Rbar.}~Amen.
}{
	{\color{red}\Vbar.} O Lord, hear my prayer.
	{\color{red}\Rbar.}~And let my cry come unto Thee.
	
	Let us pray.
	O God, who didst glorify the blessed Bishop Nicholas with innumerable miracles; grant, we beseech thee, that, by his merits and prayers, we may be saved from the fires of hell.\\
	% Through Jesus Christ, thy Son our Lord, Who liveth and reigneth with thee, in the unity of the Holy Ghost, God, world without end.
	%{\color{red}\Rbar.}~Amen.
}

\end{latinenglishsection}

%\vfill\pagebreak

%% TODO: commemorations based on the date
\section*{Commemoration of the Second Sunday of Advent}
\textit{\color{red}The Cantor intones the antiphon and leads the versicle. The Officiant prays the associated collect.}

\textit{Come, O Lord, visit us in peace, that we may rejoice before thee with all our heart.}

\gresetinitiallines{1}
\gregorioscore{../commemorations/an--veni_domine_visitare--solesmes}

\textit{\color{red}The Officiant leads the following:}

\begin{latinenglishsection}

\latinenglish{
	{\color{red}\Vbar.}~Roráte, cæli, désuper, et nubes pluant justum.\\
	{\color{red}\Rbar.}~Aperiátur terra, et gérminet Salvatórem.
	
	Orémus.
	Excita, Dómine, corda nostra ad præparándas Unigéniti tui vias: ut, per ejus advéntum, purificátis tibi méntibus servíre mereámur:\\
	Qui tecum vivit et regnat in unitáte Spíritus Sancti, Deus, per ómnia s\'{\ae}cula sæculórum.
}{
	{\color{red}\Vbar.}~Drop down dew, ye heavens, from above, and let the clouds rain the Just.\\
	{\color{red}\Rbar.}~Let the earth be opened, and bud forth the Saviour.
	
	Let us pray.
	Stir up our hearts, O Lord, to make ready the ways of thine Only-begotten Son, that by His coming our minds being purified, we may the more worthily give up ourselves to thy service:\\
	Who with thee liveth and reigneth, in the unity of the Holy Spirit, God, world without end.
}

\end{latinenglishsection}

%% TODO: benedicamus in line with the season

\textit{\color{red}The Officiant says:}

\begin{latinenglishsection}
\latinenglish{
	{\color{red}\Vbar.}~Dómine exáudi oratiónem meam.\\
	{\color{red}\Rbar.}~Et clamor meus ad te véniat.
}{
	{\color{red}\Vbar.}~Lord, hear my prayer. \\
	{\color{red}\Rbar.}~And let my cry come unto Thee.
}
\end{latinenglishsection}

\textit{\color{red}The Cantors lead the Benedicamus:}

\gresetinitiallines{1}
\gregorioscore{../common/benedicamus-advent-lent}

\textit{\color{red}The Officiant leads the following prayers:}

\begin{latinenglishsection}

\latinenglish{
	{\color{red}\Vbar.} Fidélium ánimæ, per misericórdiam Dei, requiéscant in pace. \\
	{\color{red}\Rbar.} Amen.
}{
	{\color{red}\Vbar.} May the souls of the faithful departed, through the mercy of God, rest in peace. {\color{red}\Rbar.}~Amen.
}

\latinenglish{
	Pater noster \textit{(silently)}.
}{
	Our Father\dots
}

%\latinenglish{
%	{\color{red}\Vbar.} Dóminus det nobis suam pacem. \\
%	{\color{red}\Rbar.} Et vitam ætérnam. Amen.
%}{
%	{\color{red}\Vbar.} May the Lord grant us his peace. {\color{red}\Rbar.}~And life eternal. Amen.
%}

\end{latinenglishsection}

\begin{rubricbox}

{\color{red}After the conclusion of Vespers, those vested in choir process out silently.}

\end{rubricbox}

\end{document}